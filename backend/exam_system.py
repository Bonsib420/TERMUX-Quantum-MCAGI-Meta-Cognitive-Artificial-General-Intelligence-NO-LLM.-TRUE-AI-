...
Usage in chat.py:
/exam
— Run stage advancement exam
/exam status
— Show exam readiness and history
/exam review
— Show last exam results with failures
"""
import json
import os
import time
...
return {
'stages': {},
# stage_num -> list of intake records
'current_stage': 0,
'sources': [],
# all sources ever ingested
...
■ Testing against ingested knowledge...")
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
...
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■")
■ RESULTS: {passed}/{actual_count} ({score*100:.1f}%)")
■ Threshold: {self.PASS_THRESHOLD*100:.0f}%")
■ Verdict: {'ADVANCE ✓' if advanced else 'REMAIN ✗'}")
...
print(f" ■
{ftype}: {count} failures → {diagnosis}")
print(f"
...
Score: {last['score']*100:.1f}% ({last['passed']}/{last['questions']})")
...
— Run stage advancement exam")
print(" /exam status
— Show readiness and history")
print(" /exam review
— Show last exam details")
