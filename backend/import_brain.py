...
def main():
    ...
    print(f"  Baseline KB topics:   {len(engine.knowledge.topics)}")
    print(f"  Baseline Markov states: {len(engine.markov.chain) if hasattr(engine.markov,'chain') else '?'}")
    ...
    print("\nFinal stats:")
    print(f"  TF-IDF docs:    {engine.tfidf.extractor.total_documents}")
    print(f"  KB topics:      {len(engine.knowledge.topics)}")
    print(f"  Markov states:  {len(engine.markov.chain) if hasattr(engine.markov,'chain') else '?'}")
    print(f"  Vocab terms:    {len(engine.tfidf.extractor.word_frequencies)}")
