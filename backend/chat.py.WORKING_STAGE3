...
def save_everything(memory, engine, state_dir):
    ...
    try:
        from cloud_provider import WolframCloudProvider
        c = WolframCloudProvider()
        c.save_chain('QuantumMCAGI/chain', state_dir)
        c.save('QuantumMCAGI/state', {
    ...
    try:
        from cloud_provider import PCloudProvider
        p = PCloudProvider()
        if p.save_chain(state_dir):
            print("  ☁ pCloud chain backup complete")
    except Exception as e:
        print(f"  pCloud save skipped: {e}")
...
def run_chat(verbose=False):
    ...
    if engine.load_state(state_dir):
        ...
        try:
            from cloud_provider import WolframCloudProvider, PCloudProvider
            c = WolframCloudProvider()
            if c.load_chain('QuantumMCAGI/chain', state_dir):
                print("  ☁ Chain restored from Wolfram Cloud")
                engine.load_state(state_dir)
            else:
                p = PCloudProvider()
                if p.load_chain(state_dir):
                    print("  ☁ Chain restored from pCloud")
                    engine.load_state(state_dir)
        except Exception as e:
            print(f"  Cloud restore skipped: {e}")
            ...
            if cmd[0] == '/quit':
                ...
                if HAS_EXAM:
                    ...
                    if len(cmd) > 1 and cmd[1] == 'status':
                        ...
                        runner.run_exam(0)  # current stage
                else:
