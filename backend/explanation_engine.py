...
lines.append(f"
{c}: tf={tf:.3f} idf={idf:.2f} score={score:.4f}")
else:
lines.append("
(no significant concepts extracted)")
lines.append(f"
lines.append(f"

Total vocabulary: {len(engine.extractor.word_frequencies)} words")
Corpus size: {engine.extractor.total_documents} documents")
...
Status: ACTIVE")
lines.append(f"
Conscious moments: {status.get('conscious_moments', 0)}")
lines.append(f"
Temperature: {status.get('last_temperature', 0):.3f}")
...
lines.append("
(Orch OR not loaded — using probabilistic selection)")
return "
".join(lines)
...
lines.append(f"
Register: {tone_result.get('register', 'unknown')}")
lines.append(f"
Depth: {tone_result.get('depth', 0):.3f}")
lines.append(f"
Deep markers found: {tone_result.get('deep_markers', 0)}")
lines.append(f"
Word count: {tone_result.get('word_count', 0)}")
sentiment = tone_result.get('sentiment', {})
if sentiment:
lines.append(f"
VADER sentiment:")
lines.append(f"
positive={sentiment.get('positive', 0):.3f} "
...
lines.append(f"
Available levels: {' -> '.join(level_names[:max_level+1])}")
lines.append(f"
...
lines.append(f"
Transitions: {engine.markov.total_tokens}")
lines.append(f"
Trained: {engine.markov.trained}")
...
Known words (transitions): {', '.join(known[:10])}")
Unknown words: {', '.join(unknown[:10])}")
...
lines.append("
Selected: HYBRID QUANTUM GENERATOR")
lines.append("
Reason: Deep register + hybrid available")
lines.append("
Process:")
lines.append("
1. Markov generates 8 candidate sentences")
lines.append("
2. TF-IDF scores each for topic relevance")
lines.append("
3. Coherence + coverage + length scoring")
lines.append("
4. Score = relevance*0.35 + coverage*0.25 + coherence*0.25 + length*0.15")
lines.append("
5. Orch OR collapse biases selection (if active)")
lines.append("
6. WordNet synonym swap on winner")
else:
lines.append("
Selected: TONE-AWARE COMPOSER")
lines.append(f"
...
Process:")
lines.append("
1. Markov-assisted opening sentence")
lines.append("
2. Concept threading into structured response")
lines.append("
3. Bloom's question appended if appropriate")
return "
".join(lines)
...
lines.append(f"
Score: {understanding.get('understanding_score', 0):.2f}")
...
Known concepts: {', '.join(known)}")
New concepts: {', '.join(new)}")
...
if related:
lines.append(f"
Related: {', '.join(r.get('concept', str(r)) for r in related)}")
return "
".join(lines)
...
lines.append(f"
Quote engine: {'ACTIVE (20% chance)' if has_quotes else 'OFF'}")
lines.append(f"
Personality: {'ACTIVE (30% chance)' if has_personality else 'OFF'}")
lines.append(f"
Philosophical asides: {'15% chance' if has_quotes else 'OFF'}")
lines.append(f"
Dream fragments: {'10% chance' if has_quotes else 'OFF'}")
return "
".join(lines)
