...
2. UTF-8 BOM markers
(editors injecting EF BB BF)
3. Auto-formatter mangling
(literal "\n" replacing real newlines)
4. Incomplete syncs / partial writes (caught via SHA-256 manifest)
5. Clipboard paste corruption
(NUL bytes, smart-quote substitution)
Commands (run from project root):
python file_integrity.py scan
python file_integrity.py verify
python file_integrity.py repair
python file_integrity.py status

# build manifest
# report drift vs manifest
# normalize + atomic rewrite
...
fixed += 1
print(f"
FIXED: {path.relative_to(REPO_ROOT)} [{', '.join(actions)}]")
else:
clean += 1
...
print(f"[status] root
: {REPO_ROOT}")
print(f"[status] text files
: {n}")
print(f"[status] manifest entries : {len(manifest)}")
print(f"[status] BOM contamination: {bom}")
print(f"[status] CRLF endings
: {crlf}")
print(f"[status] mixed endings
: {mixed}")
