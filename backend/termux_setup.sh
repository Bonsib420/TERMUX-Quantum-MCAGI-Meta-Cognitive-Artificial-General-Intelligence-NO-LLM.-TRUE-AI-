...
# ---------------------------------------------------------------------------
# termux_setup.sh — installs file-corruption defenses into the CURRENT
# directory (designed for Termux, but works on any Linux/macOS/WSL too).
#
# Usage from Termux:
#   cd ~/Quantum_MCAGI_NO_LLM_V⁰²/backend
#   bash termux_setup.sh
#
# Creates three files in the current directory:
#   .gitattributes      forces LF + UTF-8 at the git layer
#   .editorconfig       same enforcement at the editor layer
#   file_integrity.py   scan / verify / repair tool
# ---------------------------------------------------------------------------
set -euo pipefail

ROOT="$(pwd)"
echo "[setup] target directory: $ROOT"

# ---------------------------------------------------------------------------
# 1. .gitattributes  (append if exists, create if not)
# ---------------------------------------------------------------------------
GA="$ROOT/.gitattributes"
if [ -f "$GA" ] && grep -q "text=auto eol=lf" "$GA"; then
  echo "[setup] .gitattributes already hardened — skipping"
else
  cat >> "$GA" <<'GITATTR'

# --- universal line-ending + encoding policy (auto-installed) ---
* text=auto eol=lf
*.py     text eol=lf
*.pyi    text eol=lf
*.js     text eol=lf
...
if [ -f "$EC" ]; then
  echo "[setup] .editorconfig already exists — skipping"
else
  cat > "$EC" <<'EDITORCFG'
root = true
...
echo ""
echo "============================================================"
echo " DONE. Next steps in Termux:"
echo "   cd $ROOT"
echo "   python file_integrity.py status    # quick health check"
echo "   python file_integrity.py scan      # build SHA-256 manifest"
echo "   python file_integrity.py repair    # auto-fix anything broken"
echo "   python file_integrity.py verify    # later, detect any drift"
echo "============================================================"
