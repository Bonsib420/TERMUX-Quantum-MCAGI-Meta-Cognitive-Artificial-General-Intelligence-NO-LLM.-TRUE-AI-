...
[5] Internet search
[6] KB recheck
[7] LANGUAGE STYLIZATION

[8] Save + return
"""

— user message received
— fast lookup in fact_cache
— fuzzy match via Hilbert embeddings (find conceptually-related entries)
— gdrive MCAGI_BRAIN lookup
— auto-detected questions or explicit !search
— second pass after search may have stored a fresh answer
— Takes the retrieved truth +
runs it through Hilbert/Markov to re-express
in Quantum MCAGI's voice
— persist state, build collapse analysis dashboard
...
proj = (
f" ■
[PROJECTION]: FROM {concepts[0].upper()} TO "
f"{concepts[1].upper()}: "
