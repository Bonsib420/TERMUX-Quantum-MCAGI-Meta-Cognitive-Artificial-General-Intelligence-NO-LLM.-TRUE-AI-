...
• Sampling a token = Born rule: P(t) = |■v■ | ψ■|².
...
• tokens
— array of token strings (vocabulary)
• vectors — (vocab_size, dim) state embeddings, |token■■ = vectors[i]
• U
— (dim, dim) unitary evolution operator
Public API:
HilbertEngine(dim=128)
.load_state(path) / .save_state(path)
.sample_token(context_tokens, temperature=1.0) -> str
.evolve(token)
# advance state with U after seeing token
.reset_state()
# reset to maximally-mixed ρ
.get_status() -> dict
"""
import os
import json
...
rho = self.rho
# Born rule: P(i) = ■v■ | ρ | v■■, real and non-negative
...
# Collapse: project ρ onto the chosen state |v■■
