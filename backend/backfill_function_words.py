...
1. ~/training/math/                — collected math corpus (~102 files)
2. ~/training/                     — anything else under training
...
Usage:
    ...
    python backfill_function_words.py --dry-run        # report counts only
    python backfill_function_words.py --limit 20       # first 20 files
"""
import os
import sys
import argparse
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="report what would be ingested without saving")
    parser.add_argument("--limit", type=int, default=0, help="process at most N files (0 = all)")
    parser.add_argument("--skip-conversations", action="store_true", help="don't include past chat history")
    ...
    sources = []
    for root in [ Path(os.path.expanduser("~/training/math")),
        ...
        if conv_path.exists():
            try:
                ...
                if conv_text:
                    ...
                    print(f" ✓ conversations.json: " f"{len(conv_text)} entries ({len(combined):,} chars)")
            except Exception as e:
    ...
    print(' python -c "from function_word_engine import FunctionWordEngine;' ' e=FunctionWordEngine(); e.load(\\"' + str(OUTPUT) + '\\");'
