...
class OrchOREngine:
    ...
    def get_hilbert_amplitudes(self) -> Optional[List[float]]:
        """Joined Born-rule amplitude vector across every microtubule.
        Returned to the language pipeline so each candidate next-token can
        be weighted by the substrate's current quantum state. Returns None
        when no quantum backend is active (caller must handle gracefully)."""
        if self.quantum_engine:
            try:
                return self.quantum_engine.get_hilbert_amplitudes()
            except Exception:
                return None
        return None
    ...
    def get_status(self) -> Dict:
        if self.quantum_engine:
            # PennyLane already reports its own (live) gap-junction graph;
            # do NOT overwrite it with this wrapper's classical-fallback dict
            # or the API will report a stale 6-edge graph instead of the
            # actual substrate graph (8 systems / 16 edges as of 2026-04-30).
            return self.quantum_engine.get_status()
