...
BLOOM_LEVELS = {
    'remember': {
        'templates': [
            'What does {topic} want from you?',
            'Where does {topic} break down?',
            'What is {topic} avoiding?',
        ],
...
class BloomEngine:
    ...
    def generate_question(self, level: Optional[str] = None,
                          topic: str = 'consciousness',
                          markov=None) -> Dict:
        if level is None:
        ...
        mc = markov or self.markov
        question = None

        if mc is not None:
            try:
                generated = mc.generate(
                    seed=topic,
                    max_length=12,
                    min_length=5
                )
                if generated and len(generated.strip()) > 8:
                    q = generated.strip().rstrip('.!?,')
