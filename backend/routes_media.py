...
svg_lines.append(f'<circle cx="{x}" cy="{y}" r="{radius}" fill="hsla({hue}, {saturation}%, {lightness}%, {alpha:.2f})&quot
...
svg_lines.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="hsla({hue}, {saturation}%, {lightness}%

# Overlay prompt as subtle text
text_y = height - 50
for line in [request.prompt[:60] + ('...' if len(request.prompt)>60 else '')]:
svg_lines.append(f'<text x="50%" y="{text_y}" fill="rgba(255,255,255,0.3)" font-size="24" text-anchor="middle&qu
text_y += 30
