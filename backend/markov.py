...
class QuantumMarkovEngine:
    ...
    def save_state(self):
        ...
        try:
            ...
            if not self.silent:
                print(f"Markov saved: {len(self.chain)} states")
        except Exception as e:
            print(f"Markov save failed: {e}")
    ...
    def generate_from_concepts(self, concepts, length=30, wild=False, **kwargs):
        ...
        if not hasattr(self, "_hilbert"):
            self._hilbert = None  # Disabled — einsum too slow on ARM
        ...
        markup = {'|isbn', '|page', '{{cite', '}}</ref', '|doi', '|pmid', '|volume', '|journal'}
        import random as _random
        sample = _random.sample(list(self.chain.keys()), min(2000, len(self.chain)))
        for pre in sample:
            pre_str = ' '.join(pre).lower()
            if any(m in pre_str for m in markup):
                continue
            score = sum(1 for c in concept_set if c in pre_str)
            if score > best_score:
                best_score = score
                best_seed = pre
            if best_score >= 2:
        ...
        for _ in range(length * 2):
            ...
            _bad_tokens = {
                '{{', '}}', '|isbn', '|doi', '[[', ']]', '<ref', '</ref',
                'import ', 'def ', 'class ', '#!/', '.py',
                'http://', 'https://', '&nbsp;', '%7C', 'bgcolor', 'colspan',
            }
            if any(b in str(nxt) for b in _bad_tokens) or len(str(nxt)) > 50:
                continue
            result.append(nxt)
